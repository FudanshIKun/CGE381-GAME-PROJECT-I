...
Properties {
    ...
    _Fog2Col("Fog 2 Colour", Color) = (.5, .5, .5, .5)
    _Fog2Start("Fog 2 Begin", Range(0,1)) = 0
    _Fog2End("Fog 2 End", Range(0,1)) = .4
    _Fog2Intens("Fog 2 Density", Range(0,1)) = 1
...
SubShader {
    ...
    Pass {
        ...
        fixed4 frag (v2f i) : SV_Target
        {
            ...
            //Apply Second Fog
            half3 fog2Col = _Fog2Col.rgb;
            c = lerp(c, fog2Col, saturate((1-smoothstep(_Fog2Start*2, _Fog2End*2, (_FogBottom==0?abs(i.texcoord.y):i.texcoord.y)))*_Fog2Intens));
            c *= _Exposure;
            return half4(c, 1);
        }
        ENDCG
